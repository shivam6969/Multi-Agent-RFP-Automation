...
def router_node(state: AgentState) -> AgentState:
    ...
    if route not in ("rfp", "bu", "match", "price", "full"):
        route = "full"
...
def _route_after_router(state: AgentState) -> str:
    ...
    if route == "bu":
        return "bu_agent"
    # All other routes start with the RFP agent
    return "rfp_agent"
...
def build_graph() -> StateGraph:
    ...
    # BU agent → synthesise → END
    graph.add_edge("bu_agent", "synthesise")
    ...
    # Pricing agent → synthesise → END
    graph.add_edge("pricing_agent", "synthesise")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print('  python master_agent.py "Full RFP analysis with pricing"')
        sys.exit(1)
